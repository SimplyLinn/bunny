...
/*
 * Modified by Linn to remove all references to the global Guacamole
 * object. Original, unmodified, source can be located at
 * https://github.com/apache/guacamole-client/blob/master/guacamole-common-js/src/main/webapp/modules/Keyboard.js
 * 
 * please note that this module is under a different license than the rest of this project.
 * it will most likely be refactored into its own standalone module to ease the licensing issues.
...
function Keyboard(element) {
  ...
  this.listenTo = function listenTo(element) {
      if (guac_keyboard.isAttached) guac_keyboard.detach;
      var listeners = {};
      // When key pressed
      element.addEventListener("keydown", listeners.keydown = function(e) {
      ...
      // When key pressed
      element.addEventListener("keypress", listeners.keypress = function(e) {
          // Only intercept if handler set
      ...
      // When key released
      element.addEventListener("keyup", listeners.keyup = function(e) {
          // Only intercept if handler set
      ...
      var handleInput = function handleInput(e) {
          ...
          if (e.data && !e.isComposing) {
              delete listeners.compositionend;
      ...
      element.addEventListener("input", listeners.input = handleInput, false);
      element.addEventListener("compositionend", listeners.compositionend = handleComposition, false);
      guac_keyboard.isAttached = true;
      guac_keyboard.detach = function detach() {
          if(!guac_keyboard.isAttached) return;
          for([event, listener] of Object.entries(listeners)) {
