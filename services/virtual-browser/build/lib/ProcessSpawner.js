...
class ProcessSpawner {
  ...
  _getStdioOptions(env) {
    return {
      ...
      stdio: [typeof this.input === 'string' && this.input || this.input ? 'pipe' : 'ignore', typeof this.output === 'string' && this.output || this.output ? 'pipe' : 'ignore', typeof this.error === 'string' && this.error || this.error ? 'pipe' : 'ignore'],
  ...
  _attachListeners(process) {
    [[process.stdout, this.output], [process.stdin, this.input], [process.stderr, this.error]].forEach(([stream, listeners]) => {
  ...
  spawn(env, args = [], opts = {}) {
    const spawnPromise = new Promise((res, rej) => {
      ...
      args = this.sudo ? [this.command, ...args] : args; // this.log(cmd, args)

      this.log(cmd, args.join(" "));

      const process = this._attachListeners(_child_process.default.spawn(cmd, args, opts));
...
exports.default = ProcessSpawner;